...
import packageJson from 'azhn/package.json'
      ...
      if (response.status === 200) {
        ...
        const decodeFontInfoFromArrayBuffer = (buffer: ArrayBuffer) => {
          ...
          while (offset < bufferView.length) {
            ...
            fonts.push({
              name: `azhn_${languageCode}_fallback_${text}`,
              data: fontData,
              weight: 400,
              style: 'normal',
...
const LiveAzhn = withLive(function ({
  live,
}: {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      if (live?.element && options) {
        ...
        if (renderType !== 'html') {
          try {
            _result = await azhn(live.element.prototype.render(), {
              ...options,
              embedFont: fontEmbed,
              width,
              height,
  ...
  useEffect(() => {
    try {
      const hasVisited = localStorage.getItem(
        '_ekjotsinghmakhija_og_playground_visited'
      )
