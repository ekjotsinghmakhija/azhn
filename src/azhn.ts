...
import type { AzhnNode } from './layout.js'
...
export type { AzhnNode }

export default async function azhn(
  element: ReactNode,
  options: AzhnOptions
): Promise<string> {
  ...
  if (!Yoga || !Yoga.Node) {
    throw new Error(
      'Azhn is not initialized: expect `yoga` to be loaded, got ' + Yoga
    )
  }
...
function getRootNode(
  Yoga: TYoga,
  pointScaleFactor?: AzhnOptions['pointScaleFactor']
) {
  if (!pointScaleFactor) {
