...
export interface LayoutContext {
  ...
  onNodeDetected?: (node: AzhnNode) => void
}

export interface AzhnNode {
  // Layout information.
  left: number
  top: number
  width: number
...
> {
  ...
  if (!isReactElement(element) || typeof element.type === 'function') {
    ...
    if (!isReactElement(element)) {
      ...
      // If it's a custom component, Azhn strictly requires it to be pure,
  ...
  if (props && hasDangerouslySetInnerHTMLProp(props)) {
    throw new Error(
      'dangerouslySetInnerHTML property is not supported. See documentation for more information https://github.com/ekjotsinghmakhija/azhn#jsx.'
    )
  }
  ...
  // Post-process styles to attach inheritable properties for Azhn.
  ...
  if (
    ...
    newInheritableStyle._inheritedClipPathId = `azhn_cp-${id}`
    newInheritableStyle._inheritedMaskId = `azhn_om-${id}`
  }

  if (computedStyle.maskImage) {
    newInheritableStyle._inheritedMaskId = `azhn_mi-${id}`
  }
